...
//static fintess_svm *bio=nullptr;
fitness_svm::fitness_svm(char *filename, int num_folds)
{
    init_data_set(filename, num_folds);
}
fitness_svm::~fitness_svm()
{
...
//------------------------------------------------------------------------------------------------
void fitness_svm::init_data_set(char *filename, int num_flods)
{
    data_count=0;
    num_features=0;
...
////------------------------------------------------------------------------------------------------
//void fintess_svm::bioinfo_problem_fitness_function(individual *_individual) {
//    bio->transfer_to_svm_data(_individual);
//}
void fitness_svm::fitness_function(individual *_individual)
{
    int value=0;
    ...
    int loc=0;
    int dest_loc=0;
    param.C = _individual->get_parameters_value(index);
    index++;
    param.gamma = _individual->get_parameters_value(index);
    index++;
    loc=0;
    dest_loc=0;
...
void fitness_svm::fitness_function2(individual *_individual)
{
    int value=0;
    fitness_function_data *data=_individual->get_fitness_function_data();
    int index=0;
...
/*    k=10;
    ...
    */
    ...
    eval.m_fintess_value=100*(eval.m_multi_objs[0])+4*(1-eval.m_multi_objs[3]/(float)num_features);
...
//------------------------------------------------------------------------------------------------
//載入資料
void fitness_svm::load_training_data_set(char *filename)
{
    int i;//,index;
//    char title_name[128];
    float value;
...
//------------------------------------------------------------------------------------------------
void fitness_svm::read_data_to_memory(FILE *fp)
{
    char *p=nullptr;
    float std_value=0;
    char *pointer;
...
//------------------------------------------------------------------------------------------------
char* fitness_svm::readline(FILE *input)
{
    int len;
...
//------------------------------------------------------------------------------------------------
void fitness_svm::init_svm(float c, float gamma)
{
//    int i;
    ...
    param.kernel_type = POLY;//POLY;//RBF;
    param.degree = 3;
 //   param.gamma = 0;    // 1/num_features
    ...
    target = svm_malloc(prob.l*2, double);
    _index = svm_malloc(prob.l, int);
}
//------------------------------------------------------------------------------------------------
double fitness_svm::do_cross_validation()
{
    int i;
