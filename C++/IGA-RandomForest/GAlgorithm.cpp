...
void GAlgorithm::init(STOP_CONDITION mode,int stop_condition_parameter, int poputlation_size, float selection_rate, float crossover_rate, float mutation_rate, int parameter_count, int encode_bits_count, interface_fitness *arg, float lower_bound, float upper_bound, bool want_bigger_fitness_value)
{
    stop_mode=mode;
    ...
    m_fitness_function_data->init(parameter_count, lower_bound, upper_bound, encode_bits_count, 1, m_want_bigger_fitness_value, arg);
...
void GAlgorithm::check(double &best_value, std::string step_name)
{
    if(best_value<m_population->get_best_fitness_value().m_fintess_value)
    {
        
        best_value=m_population->get_best_fitness_value().m_fintess_value;
    }else if(best_value>m_population->get_best_fitness_value().m_fintess_value)
    {
        printf("[error] %f, %f=>%s \n", best_value, m_population->get_best_fitness_value().m_fintess_value, step_name.c_str());
        int ii=m_population->get_best_individual_index();
        individual *ind=m_population->get_individual(ii);
        fitness_function_data *data=ind->get_fitness_function_data();
        data->evalute(ind);
        ind->dump();
    }
}
...
//            evalute();
            select();
            //check(best_value, "after selection");
            crossover();
            //check(best_value, "after crossover");
            mutate();
            //check(best_value, "before mutate");
            evalute();
            //check(best_value, "after mutate");
