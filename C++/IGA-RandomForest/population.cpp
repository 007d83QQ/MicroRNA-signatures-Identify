...
{
    return m_fitness_function_data->evalute_object!=nullptr;
}
...
void population::crossover(int first, int second)
{
    ...
    m_individuals[first]->dump();
    m_individuals[second]->dump();
    best_solution->dump();
#endif
    if(m_crossover_operator->crossover(m_individuals[first], m_individuals[second], best_solution))
    {
#ifdef DEBUG_MODE
        printf("change best %d , old one %d \n", first, m_best_solution_index);
#endif
        m_best_solution_index=first;
        m_sorted_index[m_best_value_location]=first;
...
//    m_fitness_function_data->evalute(m_individuals[i]);
    ...
    for(int i=0;i<m_individuals_count;i++)
    {
        ...
        if(i==0)
        {
            best_fitness=m_individuals[0]->m_evulation_value.m_fintess_value;
        }else if(m_individuals[i]->m_evulation_value.m_fintess_value>best_fitness)
        {
            best_fitness=m_individuals[i]->m_evulation_value.m_fintess_value;
            best_index=i;
        }
    }
    if(best_index!=m_best_solution_index)
    {
        m_best_solution_index=best_index;
        m_sorted_index[m_best_value_location]=best_index;
    }
...
{
    m_best_solution_index=get_best_individual_index();
