...
public:
    ...
    virtual void dump();
private:
    void check(double &best_value, std::string step_name);
public:
    void init(STOP_CONDITION mode,int stop_condition_parameter, int poputlation_size, float selection_rate, float crossover_rate, float mutation_rate, int parameter_count, int encode_bits_count, interface_fitness *arg, float lower_bound, float upper_bound, bool want_bigger_fitness_value);
