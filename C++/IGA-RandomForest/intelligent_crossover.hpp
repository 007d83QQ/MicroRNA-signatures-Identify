...
private:
    ...
    void recombine_parameters(int factor_index, std::vector<int> cut_point, individual *source_individuals, individual *target_individuals, individual *target_individuals_two=nullptr);
    void found_two_best_value(float *fintess_vlaue, int *best_value_index, bool want_bigger=true);
 //   int reduce_OA_table_size(individual *parent1, individual *parent2, std::vector<int> &different_pos);
    int reduce_OA_table_size(individual *parent1, individual *parent2, std::vector<int> &different_pos, int &current_oa_height, int &current_oa_width, int &oa_jump_distance);
private:
